...
public class DriverBaseSalaryService {
    ...
    private DriverBaseSalaryResponse toResponse(DriverBaseSalary salary) {
        return DriverBaseSalaryResponse.builder()
                ...
                .driverLicenseId(salary.getDriverLicense() != null ? salary.getDriverLicense().getId() : null)
                .licenseNumber(salary.getDriverLicense() != null ? salary.getDriverLicense().getLicenseNumber() : null)
                .licenseClass(salary.getDriverLicense() != null ? salary.getDriverLicense().getLicenseClass() : null)
    ...
    public DriverBaseSalaryResponse updateBaseSalary(Integer id, DriverBaseSalaryRequest request) {
        DriverBaseSalary salary = driverBaseSalaryRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy lương cứng"));
